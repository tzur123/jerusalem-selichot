        ...
        >
          ...
          {onClose && (
            <button
              ...
              onClick={onClose}
              aria-label="סגירה"
              className="absolute top-5 end-5 z-10 flex h-11 w-11 items-center justify-center rounded-full bg-navy/60 text-white/90 hover:text-white transition-colors"
            >
              <CloseIcon />
            </button>
          )}
          <div className={cn(headerImage && "px-6 pt-4")}>
            {title && <h2 className={cn("text-xl font-bold mb-2 font-heading", onClose && "pe-14")}>{title}</h2>}
            {children}
          </div>
        </div>
      </div>
